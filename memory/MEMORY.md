...
- [Respuesta por talla (028) — PR #31](respuesta-por-talla-028-pr31.md) — PR #31 (4d3662f) + ajuste de orden PR #32 (71e9abf: espera el `sent` de la foto anterior, tope 2 s) desplegados y verificados por WhatsApp con los 4 modelos reales el 2026-09-17 (cierra la T057); NO promovida; gotchas del arnés: base nueva por corrida, calentar rutas, no resetear el outbox
- [El self-test de la agenda en local necesita las URLs de los mocks](agenda-e2e-local-necesita-urls-de-mocks.md) — el `.env` de la máquina no trae AGENDA ni ZOOM_*/GOOGLE_* → el bloque Zoom da 5 FAIL por `invalid_client` contra el Zoom real; `.env.local` temporal + exportar en la shell del arnés
