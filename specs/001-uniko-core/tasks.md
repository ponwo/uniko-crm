# Tasks: Uniko CRM — Núcleo v1

**Input**: Design documents from `/specs/001-uniko-core/`
...
- [X] T075 [US8] Seed "Ferretería El Martillo": scripts/seed/demo.mjs + ~~POST /api/seed/demo + botón en UI~~ → `POST /api/dev/seed-demo`, sin botón (**DEROGADO EN PARTE** por [023](../023-seed-demo-fuera-de-produccion/spec.md)) (solo BD de dominio vacía) — ~8 contactos MX, conversaciones con cotizaciones MXN, pipeline poblado, KB llena con huecos INTENCIONALES (garantías y devoluciones), 1 corrida de Laboratorio de ejemplo guardada; idempotente (DELETE org demo por orden de FKs)
...
- [X] T078 [P] [US8] LICENSE (MIT) + revisión de metadata pública de package.json (name uniko-crm, license MIT)
...
- [X] T098 Merge `001-uniko-core` → `main` (merge normal, conservar rama — OK explícito ya dado), validar con /speckit-git-validate, dejar `main` activa
